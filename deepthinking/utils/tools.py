...
def get_model(model, width, max_iters, in_channels=3, noise_type='none', noise_block_idx=1):
    model = model.lower()
    #changed!
    #net = getattr(models, model)(width=width, in_channels=in_channels, max_iters=max_iters)
    net = getattr(models, model)(width=width, in_channels=in_channels, max_iters=max_iters, noise_type=noise_type, noise_block_idx=noise_block_idx)
    print(f"Using model {model} with noise_type {noise_type} and noise_block_idx {noise_block_idx}.")
    return net
...
def load_model_from_checkpoint(problem, model_args, device):
    ...
    noise_block_idx = model_args.noise_block_idx
    epoch = 0
    optimizer = None
    ...
    net = get_model(model, width, in_channels=in_channels, max_iters=max_iters, noise_type=noise_type, noise_block_idx=noise_block_idx)
    net = net.to(device)
    if device == "cuda":
